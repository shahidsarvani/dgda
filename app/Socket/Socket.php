<?php
	...
	class SocketServer
	{
		...
		{
			set_time_limit(0);
			SocketServer::debug("set_time_limit<br />");
			$this->hooks = array();
			SocketServer::debug("hook array initialized<br />");

			$this->config["ip"] = $bind_ip;
			$this->config["port"] = $port;
			
			SocketServer::debug("Variables set<br />");

			$this->master_socket = socket_create(AF_INET, SOCK_STREAM, 0);
			$bResult = socket_bind($this->master_socket,$this->config["ip"],$this->config["port"]) or die("Issue Binding");
			SocketServer::debug("Bound on {$bind_ip}:{$port} " . $bResult . "<br />");
			$bResult = socket_getsockname($this->master_socket,$bind_ip,$port);
			SocketServer::debug("getSockName Reuslt: " . $bResult . "<br />");
			socket_listen($this->master_socket);
			SocketServer::debug("Listenting for connections on {$bind_ip}:{$port}" . "<br />");
		}
