...
@section('content')
    ...
    <div class="mb-3 pt-2">
        <div class="row">
            ...
            <div class="col-md-6">
                <a href="{{ route('projectormedia.create') }}" type="button" class="btn btn-primary float-right"><i
    ...
    <div class="col-md-12">
            <div class="card">
                <div class="card-header header-elements-inline">
                    <h5 class="card-title">Projector Media</h5>
                </div>
                <div class="table-responsive">
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                <th colspan="6"> 
                                    <form method="GET" action="{{ route('projectormedia.index') }}" class="mb-3">
                                        <div class="input-group">
                                            <input type="text" name="search" class="form-control" placeholder="Search..." value="{{ request('search') }}">
                                            <span class="input-group-append">
                                                <button class="btn btn-primary" type="submit">Search</button>
                                                <button class="btn btn-link" type="button" onclick="window.location='{{ route('projectormedia.index') }}'">Clear</button>
                                            </span>
                                        </div>
                                    </form>
                                </th>
                            ...
                            @if (!$media->isEmpty())
                                @foreach ($media as $k => $item)
                                    <tr>
                                        ...
                                        <td> <a class="media-modal-cls" data-media-item="{{ $item->id }}" data-toggle="modal" data-target="#media_remote_modal" href="javascript:void(0);">
                                                {{ $item->name }}
                                            </a> 
                                        </td>
                                        <td>{{ $item->lang }}</td>
